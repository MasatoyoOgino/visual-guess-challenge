...
class MainWindow(QMainWindow):
    ...
    def on_mode_selected(self):
        ...
        if selected_mode:
            self.message_label.setText(f"メッセージ：{selected_mode.upper()}モードが選択されました")
    ...
    def update_display(self):
        ...
        # 画像表示
        processed_image = self.game_engine.get_current_image()
        if processed_image is not None:
            self.display_image(processed_image)
        
        # ヒントレベル表示
        level = self.game_engine.get_current_level()
        self.update_hint_display(level)
    ...
    def submit_answer(self):
        ...
        if is_correct:
            ...
            QMessageBox.information(self, "正解！", f"正解です！\nスコア：{score:.1f}点")
        else:
            self.message_label.setText(f"残念！正解は「{correct_answer}」でした。")
            QMessageBox.warning(self, "不正解", f"残念！正解は「{correct_answer}」でした。")
    ...
    def load_random_image(self):
        ...
        self.update_display()
        self.update_timer.start(100)  # 100msごとに更新
        self.message_label.setText(f"メッセージ：ランダム画像を読み込みました（{os.path.basename(image_path)}）")
